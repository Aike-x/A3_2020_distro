...
int ingredient_index(char source_ingredient[MAX_STR_LEN])
{
    ...
    for (i = 0; i < MAT_SIZE; i++){
      // when the ingredients matches the source ingredents then return the index.
      if (strcmp(ingredients[i], source_ingredient) == 0){
        return i;
      }
    }
    //if not find then return -1 as require.
    return -1;
}
...
void related_ingredients(char source_ingredient[MAX_STR_LEN])
{
    ...
    //AdjMat[i][j] = 0 if two ingredients i and j are not directly linked
    //AdjMat[i][j] > 0 if two ingredients i and j are neighbours

    int i;
    int col;
    // use the helper function to find the index that source ingredient at.
    i = ingredient_index(source_ingredient);
    // when i is -1 there is no such a ingredent in list.
    if(i != -1){
      //find the nei.. by finding the clou is not equal to 0
    ...
    //there is never be a ingredent in the list then return nothing.
    if(i == -1) return;
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
    ...
    // the depth of the recusion call is k.
    if (dist - 1 != k){
      if (i != -1){// only if index is been found.
        for (int col = 0; col < MAT_SIZE; col++) {
          if (AdjMat[i][col] > 0) {
            // only add the neighour index to the list if it has not already been added
            if (searchInt(h, col) == 0){
              h = insertInt(h, col);
            }
...
intNode *helper_function_for_related_with_restrictions(intNode*src_h, intNode*avd_h, char avoid[MAX_STR_LEN]){
  intNode *h = NULL;

  intNode *curr = src_h;
  while (curr != NULL) {
    // check if curr is not in avd_h
    if (searchInt(avd_h, curr->x) == 0 && curr->x != ingredient_index(avoid)){
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
    ...
    intNode *src_h = NULL;
    src_h = related_k_dist(NULL, source_ingredient, k_source, 0);

    intNode *avd_h = NULL;
    avd_h = related_k_dist(NULL, avoid, k_avoid, 0);

    intNode *h = NULL;
    h = helper_function_for_related_with_restrictions(src_h, avd_h, avoid);
...
void check_in_part_recipe(char recipe[10][MAX_STR_LEN], int sum_list[MAT_SIZE], int max, int max_index){
  int i;
  int in_recipe;
  int j;
  for (i = 0; i < MAT_SIZE; i++) {
    
    // check if the current ingriedent is part of the recipe
     in_recipe = 0;
     for (j = 0; strcmp(recipe[j], "") != 0; j++) {
     ...
     // if not, keep track of the max sum
     if (!in_recipe && sum_list[i] >= max) {
       max = sum_list[i];
       max_index = i;
     }
   }
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
   ...
   */
   ...
   int sum_list[MAT_SIZE] = {0};
   int i;
   int j;
   int col;


   // for each ingreident in the fking recipe :)
   for (i = 0; strcmp(recipe[i], "") != 0; i++) {
      // skip the to_change
      if (strcmp(recipe[i], to_change) == 0) {
        to_change_index = i; // store the index of to_change
        continue; // continue on the next iteration
      }

      // accumulate (keep track) of the sum of each related ingriedent
      for (col = 0; col < MAT_SIZE; col++){
        sum_list[col] += AdjMat[ingredient_index(recipe[i])][col];
      }
   }

   // sum_list => [3, 0, 1, 0, 3, 0, 0, 8, 0, 3]
   // 
   for (int i = 0; i < MAT_SIZE; i++) {
